...
def copy_password(text: str | None) -> None:
	if text is not None:
		# Execute JavaScript (WORKS ONLY ON LOCALHOST IF RAN WITHOUT HTTPS)
		st.html(f"<script>navigator.clipboard.writeText('{text}');</script>", unsafe_allow_javascript=True)
